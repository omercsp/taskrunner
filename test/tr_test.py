...
def run_test_tasks(tasks: dict, diff_output: bool, stop_on_failure: bool,
                   tasks_list: list[str], show_colors: bool) -> int:
    class Colors(object):
        BOLD = '\033[1m'
        GREEN = '\033[32m'
        ERROR = '\033[31m'
    ...
    for t_name in tasks_list:
        try:
            t_meta = tasks[t_name].get("meta", {})
        except KeyError:
            raise TaskTestException("No such task '{}'".format(t_name))
        ...
        try:
            ...
            if generated_expected_output:
                expected_file = "{}/intermediate.expected".format(SCRIPT_DIR, t_name)
            else:
...
if __name__ == "__main__":
    ...
    try:
        init()
        tasks = read_tasks()
        exec_task_list = args.task if args.task else tasks.keys()
        rc = run_test_tasks(tasks, not args.no_diff, args.stop_on_failure,
                            exec_task_list, not args.no_colors)  # type: ignore
